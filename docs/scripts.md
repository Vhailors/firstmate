...
| `fm-pi-primary.sh`       | Launch a Pi primary with discovery disabled and only the required Firstmate extensions restored |
...
| `fm-project-remove.sh`   | Captain-gated guarded removal transaction for one registered project clone and its registry line |
...
| `fm-merge-authority-lib.sh` | Own the task `merge=` field both merge actions consult before landing work        |
