...
- The tracked `operator/` package is the one exception to these shell rules: it is TypeScript with colocated `*.test.ts` and `*.test.tsx` files and its own pnpm test, lint, and build commands, owned by `operator/README.md`; `bin/fm-lint.sh` and `bin/fm-test-run.sh` do not cover it.
