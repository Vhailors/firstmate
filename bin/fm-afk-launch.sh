...
fm_afk_launch_record_write() { # <backend> <target> <extra>
	...
	mv "$pending" "$FM_AFK_LAUNCH_RECORD" || {
		rm -f "$pending"
		return 1
	}
}
...
fm_afk_launch_flag_write() {
	...
	mv "$pending" "$FM_AFK_LAUNCH_STATE/.afk" || {
		rm -f "$pending"
		return 1
	}
}
...
fm_afk_launch_submit_herdr_command() { # <session> <pane> <command>
	local session=$1 pane=$2 command=$3 attempt=0
	fm_backend_herdr_cli "$session" pane send-text "$pane" "$command" >/dev/null 2>&1 || return 1
	while [ "$attempt" -lt 3 ]; do
		attempt=$((attempt + 1))
		fm_backend_herdr_cli "$session" pane send-keys "$pane" enter >/dev/null 2>&1 || return 1
		if fm_afk_launch_wait_ready herdr "$session:$pane"; then
			return 0
		fi
		fm_afk_launch_terminal_alive herdr "$session:$pane" || return 1
	done
	return 1
}
...
fm_afk_launch_wait_herdr_shell() { # <session> <pane>
	local session=$1 pane=$2 attempt=0 process_info shell_pid
	while [ "$attempt" -lt 100 ]; do
		attempt=$((attempt + 1))
		process_info=$(fm_backend_herdr_cli "$session" pane process-info --pane "$pane" 2>/dev/null) || {
			sleep 0.05
			continue
		}
		shell_pid=$(printf '%s' "$process_info" | jq -r '.result.process_info.shell_pid // empty' 2>/dev/null) || shell_pid=""
		if [ -n "$shell_pid" ] && printf '%s' "$shell_pid" | grep -Eq '^[0-9]+$'; then
			return 0
		fi
		sleep 0.05
	done
...
fm_afk_launch_create_herdr() { # <captain-target> <captain-backend>
	...
	fm_backend_herdr_server_ensure "$session" || {
		fm_afk_launch_log "herdr server not ready for session '$session'"
		return 1
	}
	...
	cmd=$(printf 'exec /usr/bin/env FM_HOME=%q FM_SUPERVISOR_TARGET=%q FM_SUPERVISOR_BACKEND=%q %q' \
	...
	if ! fm_afk_launch_wait_herdr_shell "$session" "$pane"; then
		fm_afk_launch_log "herdr daemon pane shell did not become ready; closing $session:$pane"
		FM_AFK_REC_BACKEND=herdr
		FM_AFK_REC_TARGET="$session:$pane"
		fm_afk_launch_close_recorded || true
		return 1
	fi
	if ! fm_afk_launch_submit_herdr_command "$session" "$pane" "$cmd"; then
		fm_afk_launch_log "failed to submit daemon in herdr pane $session:$pane; closing it"
		FM_AFK_REC_BACKEND=herdr
		FM_AFK_REC_TARGET="$session:$pane"
		fm_afk_launch_close_recorded || true
		return 1
...
fm_afk_launch_start() {
	...
	if [ -f "$FM_AFK_LAUNCH_STATE/.afk" ]; then
		had_afk=1
		cp "$FM_AFK_LAUNCH_STATE/.afk" "$backup/.afk" || {
			rm -rf "$backup"
			return 1
		}
	fi
	for artifact in .subsuper-escalations .subsuper-escalations.since .subsuper-inject-wedged; do
		if [ -e "$FM_AFK_LAUNCH_STATE/$artifact" ]; then
			cp -p "$FM_AFK_LAUNCH_STATE/$artifact" "$backup/$artifact" || {
				rm -rf "$backup"
				return 1
			}
		fi
	...
	if [ "$result" -eq 0 ]; then
		...
		herdr)
			...
			;;
		tmux)
			fm_afk_launch_create_tmux "$captain_target" "$captain_backend"
			result=$?
			;;
		*)
