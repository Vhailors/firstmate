...
#   Whole metadata records are staged beside the live record and renamed over it,
#   so readers never observe a partially written authority field.
#   bin/fm-merge-authority-lib.sh owns the field and its closed-set values.
...
spawn_remote_secondmate() {
  ...
  desired_session=$(secondmate_registry_field "$DATA/secondmates.md" "$id" herdr-session)
  case "$desired_session" in
    default|fm-remote) ;;
    *)
      fm_lock_release "$registry_lock" || true
      fm_lock_release "$SPAWN_TASK_LOCK" || true
      echo "error: remote secondmate $id has an invalid Herdr session pin: ${desired_session:-missing}" >&2
      return 1
      ;;
  esac
  positional=${POS[1]:-}
  ...
  rc=0
  fm_remote_readiness_ensure "$SCRIPT_DIR" "$id" "$desired_session" || rc=$?
  if [ "$rc" -ne 0 ]; then
    fm_lock_release "$registry_lock" || true
    fm_lock_release "$SPAWN_TASK_LOCK" || true
  ...
  launch_args=("$id" "$harness" "$model" "$effort" "$backend" "$desired_session" "$expected_target")
  ...
  if [ "$remote_herdr_session" != "$desired_session" ] || [ "${remote_target%%:*}" != "$remote_herdr_session" ]; then
    fm_lock_release "$remote_lock" || true
    fm_lock_release "$registry_lock" || true
    fm_lock_release "$SPAWN_TASK_LOCK" || true
    echo "error: remote launch returned Herdr session '${remote_herdr_session:-missing}', expected '$desired_session'; preserving the remote route for reconciliation" >&2
    return 1
  fi
...
pi_dynamic_workflows_extension_path() {
  local agent_dir extension
  agent_dir=$(pi_agent_dir)
  extension=${FM_PI_DYNAMIC_WORKFLOWS_EXTENSION:-$agent_dir/git/github.com/QuintinShaw/pi-dynamic-workflows/extensions/workflow.ts}
  case "$extension" in
    /*) ;;
    *) echo "error: FM_PI_DYNAMIC_WORKFLOWS_EXTENSION must be an absolute path: $extension" >&2; return 1 ;;
  esac
  [ -f "$extension" ] || {
...
pi_crew_thin_extension_path() {
  ...
  case "$agent_dir" in
    /*) ;;
    *) echo "error: thin pi crewmate launch needs an absolute pi agent dir: $agent_dir (set PI_CODING_AGENT_DIR or an absolute HOME)" >&2; return 1 ;;
  esac
  ...
  case "$harness" in
    ...
    pi|pi-signed)
      if [ "$kind" = secondmate ]; then
        ...
        printf '%s%s%s' 'FM_PI_EXTENSION_ISOLATION=1 ' "$harness" ' --no-extensions __MODELFLAG____EFFORTFLAG____PITHIN__-e __PIWORKFLOW__ -e __PIEXT__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
      fi
      ;;
...
LAUNCH=${LAUNCH//__PITHIN__/$PI_THIN_FLAGS}
LAUNCH=${LAUNCH//__PIBACKGROUND__/$sq_pibackground}
LAUNCH=${LAUNCH//__PIRENDERCACHE__/$sq_pirendercache}
...
if [ -n "$SPAWN_TRACEPARENT" ]; then
  if spawn_send_text_line "$T" "export TRACEPARENT=$SPAWN_TRACEPARENT"; then
    SPAWN_META_TMP=$(spawn_meta_tmp "$STATE/$ID.meta" 2>/dev/null || true)
    if [ -z "$SPAWN_META_TMP" ] \
      || ! cp -- "$STATE/$ID.meta" "$SPAWN_META_TMP" \
      || ! printf 'traceparent=%s\n' "$SPAWN_TRACEPARENT" >> "$SPAWN_META_TMP" \
      || ! spawn_meta_publish "$STATE/$ID.meta" "$SPAWN_META_TMP"; then
...
echo "spawned $ID harness=$HARNESS kind=$KIND$SPAWN_DELIVERY window=$META_WINDOW worktree=$WT${MERGE_AUTHORITY:+ merge=$MERGE_AUTHORITY}"
