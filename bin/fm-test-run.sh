...
family_for_basename() {
  case "$1" in
    ...
    fm-operational-input.test.sh|fm-pi-primary.test.sh|fm-pi-primary-types.test.sh|\
    ...
    fm-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-merge-authority.test.sh|fm-pr-check-security.test.sh|fm-pr-merge.test.sh|fm-review-diff.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-timeout-lib.sh)
      ...
      printf '%s\n' pure-contract-unit
      ;;
    bin/fm-pr-*|bin/fm-merge-*|bin/fm-teardown.sh|bin/fm-review-diff.sh|\
