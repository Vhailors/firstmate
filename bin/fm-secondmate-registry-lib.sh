...
# New remote records also pin the Herdr session used for the secondmate agent:
#   (host: ...; root: ...; home: ...; herdr-session: default|fm-remote; scope: ...; projects: ...; added YYYY-MM-DD)
# The legacy form remains readable and resolves to fm-remote.
...
SECONDMATE_REGISTRY_HERDR_SESSION=
...
secondmate_registry_parse_line() {
  ...
  local remote_session_re='^- ([A-Za-z0-9._-]+) - (.+) \(host:[[:space:]]*([^;)]*);[[:space:]]*root:[[:space:]]*([^;)]*);[[:space:]]*home:[[:space:]]*([^;)]*);[[:space:]]*herdr-session:[[:space:]]*(default|fm-remote);[[:space:]]*scope:[[:space:]]*(.*);[[:space:]]*projects:[[:space:]]*([^;)]*);[[:space:]]*added[[:space:]]+([0-9]{4}-[0-9]{2}-[0-9]{2})\)[[:space:]]*$'
  SECONDMATE_REGISTRY_ID=
  SECONDMATE_REGISTRY_SUMMARY=
  SECONDMATE_REGISTRY_HOST=
  SECONDMATE_REGISTRY_ROOT=
  ...
  if [[ "$line" =~ $local_re ]]; then
  ...
  elif [[ "$line" =~ $remote_session_re ]]; then
    SECONDMATE_REGISTRY_ID=${BASH_REMATCH[1]}
    SECONDMATE_REGISTRY_SUMMARY=${BASH_REMATCH[2]}
    SECONDMATE_REGISTRY_HOST=${BASH_REMATCH[3]}
    SECONDMATE_REGISTRY_ROOT=${BASH_REMATCH[4]}
    SECONDMATE_REGISTRY_HOME=${BASH_REMATCH[5]}
    SECONDMATE_REGISTRY_HERDR_SESSION=${BASH_REMATCH[6]}
    SECONDMATE_REGISTRY_SCOPE=${BASH_REMATCH[7]}
    SECONDMATE_REGISTRY_PROJECTS=${BASH_REMATCH[8]}
    ...
    SECONDMATE_REGISTRY_ROOT=${BASH_REMATCH[4]}
    SECONDMATE_REGISTRY_HOME=${BASH_REMATCH[5]}
    SECONDMATE_REGISTRY_HERDR_SESSION=fm-remote
