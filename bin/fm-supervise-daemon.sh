...
wedge_alarm_run_bounded() {
	...
	*m*) ;;
	*)
		log "wedge alarm: ${channel} notifier skipped because its watchdog could not start"
		return 125
		;;
	esac
	"$@" &
...
wedge_alarm_via_osascript() { # <summary>
	...
	command -v osascript >/dev/null 2>&1 || {
		log "wedge alarm: osascript not found; cannot post a macOS notification"
		return 1
	}
...
wedge_alarm_via_herdr() { # <summary>
	...
	command -v herdr >/dev/null 2>&1 || {
		log "wedge alarm: herdr not found; cannot post a herdr notification"
		return 1
	}
...
wedge_alarm_via_command() { # <cmd> <summary>
	...
	[ -n "$cmd" ] || {
		log "wedge alarm: empty command: channel; nothing to run"
		return 1
	}
...
fm_super_main() {
	...
	# The lock PID and identity must describe the same process. Using BASHPID here
	# can diverge from $$ when the daemon is launched through a shell wrapper,
	# making the launcher reject a healthy daemon as not ready.
	echo "$$" >"$PIDFILE"
