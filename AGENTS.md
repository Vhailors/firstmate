...
config/pi-crew-thin  opt-in thinner Pi crewmate startup; LOCAL, gitignored, and not inherited; values 1 or on add --no-skills plus the global background-terminals and pi-render-cache extensions to normal pi ship/scout launches only, never to a pi secondmate coordinator; any other value or an absent file keeps the default launch (bin/fm-spawn.sh)
